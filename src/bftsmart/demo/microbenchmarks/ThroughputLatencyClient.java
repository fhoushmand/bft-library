...
public class ThroughputLatencyClient {
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 9) {
            System.out.println("Usage: ... ThroughputLatencyClient <initial client id> <number of clients> <number of operations> <request size> <interval (ms)> <read only?> <verbose?> <nosig | default | ecdsa> <cluster id>");
            System.exit(-1);
        }
        ...
        for(int i=0; i<numThreads; i++) {
            ...
            clients[i] = new ThroughputLatencyClient.Client(initId+i,numberOfOps,requestSize,interval,readOnly, verbose, s, clusterId);
        }
    ...
    static class Client extends Thread {
        ...
        public Client(int id, int numberOfOps, int requestSize, int interval, boolean readOnly, boolean verbose, int sign, int clusterId) {
            ...
            this.proxy = new ServiceProxy(id, clusterId);
