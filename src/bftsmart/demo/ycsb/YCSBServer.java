...
public class YCSBServer extends DefaultRecoverable {
    ...
    public static void main(String[] args) throws Exception {
        if (args.length == 2) {
            new YCSBServer(new Integer(args[0]), new Integer(args[1]));
        } else {
            System.out.println("Usage: java ... YCSBServer <replica_id> <cluster_id>");
        }
    }

    private YCSBServer(int id, int cluster) {
        this.mTables = new TreeMap<>();
        new ServiceReplica(id, this, this, cluster);
    }
    ...
    public byte[][] appExecuteBatch(byte[][] commands, MessageContext[] msgCtx, boolean fromConsensus) {
        ...
        for (byte[] command : commands) {
            ...
            if (_debug) {
                System.out.println("[INFO] Processing an ordered request: " + aRequest.toString());
            }
