 ...
 */
public class ConcurrencyTestOld {

    static public Thread test(final BFTNode node) {
    ...
    static public Thread forkNode(final int id) {
        return new Thread(new Runnable() {
            @Override
            public void run() {
                BFTNode node = new BFTNode();
                try {
                    node.open(HermesRuntime.getRandomUUID());
                } catch (Exception ex) {
                    Logger.getLogger(ConcurrencyTestOld.class.getName()).log(Level.SEVERE, null, ex);
                }
