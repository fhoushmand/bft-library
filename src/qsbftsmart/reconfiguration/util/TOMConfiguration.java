...
*/
package qsbftsmart.reconfiguration.util;
...
public class TOMConfiguration extends Configuration {
    ...
    /* Farzin Houshmand */
    // Support for quorom systems
    private ArrayList<Set<Integer>> quorums;
    private ArrayList<Set<Integer>> bs;

    private void initializeDQS()
    {
        quorums = new ArrayList<>();
        bs = new ArrayList<>();
        String Q = configs.remove("system.servers.quorums").toString();
        for(String q : Q.split("/"))
        {
            Set<Integer> quorum = new HashSet<>();
            for (String node : q.split(","))
                quorum.add(Integer.valueOf(node));
            quorums.add(quorum);
        }

        String B = configs.remove("system.servers.bs").toString();
        for(String b : B.split("/"))
        {
            Set<Integer> fail = new HashSet<>();
            for (String node : b.split(","))
                fail.add(Integer.valueOf(node));
            bs.add(fail);
        }
    }

    //TODO
    private boolean isValidQuorumSystem()
    {
        return true;
    }
    ...
    protected void init() {
        ...
        try {
            ...
            //read instanceId and remove from the config file
            String id = (String) configs.remove("system.servers.id");
            instanceId = id;
