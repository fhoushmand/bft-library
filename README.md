...
```
#server processNumber, address and port (the ids from 0 to n-1 are the service replicas) 
...
```
./runscripts/smartrun.sh bftsmart.reconfiguration.util.DefaultVMServices <smart processNumber> <ip address> <port> (to add a replica to the group)
./runscripts/smartrun.sh bftsmart.reconfiguration.util.DefaultVMServices <smart processNumber> (to remove a replica from the group)
```
...
```
./runscripts/smartrun.sh bftsmart.tom.util.RSAKeyPairGenerator <processNumber> <key size>
```
